...
import { CLINICAL_LIBRARY_DATA } from "../../entities/guideline/model";
            ...
            <div key={field.id} style={{ marginBottom: "1.25rem" }}>
              <div
                onClick={() => {
                  ...
                  if (field.conditions.length > 0) setActiveConditionId(field.conditions[0].id);
                }}
                style={{
                  fontWeight: 700, fontSize: "0.95rem", padding: "6px 8px", cursor: "pointer",
              ...
              <div style={{ marginLeft: "1.5rem", borderLeft: "1px solid var(--border)", paddingLeft: "0.5rem", marginTop: "0.25rem" }}>
                {field.conditions.map(condition => (
                  <div
                    key={condition.id}
                    onClick={() => { setSelectedFieldId(field.id); setActiveConditionId(condition.id); }}
                    style={{
                      padding: "6px 10px", fontSize: "0.85rem", cursor: "pointer", borderRadius: "4px", marginTop: "2px",
                  ...
                  >
                    ...
                    {condition.isCustomExtension && (
                      <span style={{
                        ...
                        padding: "1px 4px", borderRadius: "4px", marginLeft: "4px", fontWeight: "bold"
                      }}>
                        EXT
                      </span>
                    )}
          ...
          {activeCondition ? (
            <div>
                ...
                <div>
                  ...
                  <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                    ...
                    {activeCondition.evidenceGrade && (
                      <span className="chip" style={{ background: "#edf2f7", color: "var(--text-main)" }}>{activeCondition.evidenceGrade}</span>
                    )}
                    <span className="chip" style={{ background: "#edf2f7", color: "var(--text-muted)" }}>Updated: {activeCondition.lastUpdatedYear}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="chip-group" style={{ marginBottom: "1.5rem", borderBottom: "1px solid var(--border)", paddingBottom: "0.75rem" }}>
                {([
                  { id: "A", label: "[A] Nutrition Assessment" },
                  { id: "D", label: "[D] Nutrition Diagnosis" },
                  { id: "I", label: "[I] Nutrition Intervention" },
                  { id: "M", label: "[M/E] Monitoring & Evaluation" },
                ] as const).map(tab => (
                  <button
                    key={tab.id}
                    className={`chip ${activeNcpTab === tab.id ? "active" : ""}`}
                    onClick={() => setActiveNcpTab(tab.id)}
                  >
                    {tab.label}
                  </button>
                ))}
              ...
              <div className="card" style={{ boxShadow: "none", background: "var(--bg-color)" }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                      {activeCondition.diagnosis.map((dx, i) => (
                        <div key={i} style={{ background: "var(--white)", padding: "0.75rem 1rem", borderRadius: "6px", borderLeft: "4px solid var(--danger)", border: "1px solid var(--border)", fontSize: "0.9rem" }}>
                          {dx}
                        </div>
                      ))}
                    </div>
                ...
                {activeNcpTab === "I" && (
                  <div>
                    ...
                    <div style={{ background: "var(--white)", padding: "1.25rem", borderRadius: "6px", borderLeft: "4px solid var(--success)", border: "1px solid var(--border)", marginBottom: "1rem" }}>
