...
function PNBagMatrix({ bag, idx, onChange, onRemove, patientWtKg, ageDays }: { bag: PNFeed; idx: number; onChange: (u: PNFeed) => void; onRemove: () => void; patientWtKg: number; ageDays: number | null }) {
  ...
  const swfiVol = helper.calcSWFI(bag.fwGoalMl, aaG, aaConcPct, dexG, dexConcPct, ileG, ileConcPct, additivesVol);

  const totalVol = Math.round(aaVol + dexVol + ileVol + additivesVol + swfiVol);
  const totalKcal = Math.round(aaG * 4 + dexG * 3.4 + ileG * 10);
  const rate = totalVol > 0 ? totalVol / durationHrs : 0;
  ...
  const finalDexPct = totalVol > 0 ? (dexG / totalVol) * 100 : 0;
    ...
    <div style={{ border: "1px solid #e2e8f0", borderRadius: "7px", marginBottom: "1rem", overflow: "hidden" }}>
      ...
      {bag.expanded && (
        <div style={{ background: "#fff" }}>
          {/* Config Bar */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: "10px", padding: "10px 12px", background: "#f8fafc", borderBottom: "1px solid #e2e8f0" }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <div style={{ minWidth: "850px" }}>
              ...
              <div style={sectionHeaderStyle}>Macronutrients</div>
              ...
              <div style={rowStyle}>
                ...
                <div style={{ fontSize: "0.8rem", color: "#718096" }}>{(totalVol > 0 ? (aaG / totalVol * 100) : 0).toFixed(1)}% final</div>
              </div>
              ...
              <div style={rowStyle}>
                ...
                <div style={{ fontSize: "0.75rem", color: lipidPctKcal > 30 ? "#e67e22" : "#718096", fontWeight: lipidPctKcal > 30 ? 700 : 400 }}>
              ...
              <div style={{ ...rowStyle, borderTop: "2px solid #2c3e50", fontWeight: 800, background: "#f8fafc" }}>
                <div>Total PN Bag<div style={{ fontSize: "0.62rem", fontWeight: 400, color: "#718096" }}>Wt: {patientWtKg} kg</div></div>
                <div>—</div>
                <div>{patientWtKg > 0 ? (totalKcal / patientWtKg).toFixed(1) : "—"} kcal/kg</div>
                <div style={{ fontSize: "1rem" }}>{totalKcal} kcal</div>
                <div style={{ fontSize: "0.75rem", color: "#718096" }}>Total Volume</div>
                <div style={{ fontSize: "1rem" }}>{totalVol} mL</div>
                <div>{totalVol > 0 && <span style={{ fontSize: "0.85rem", color: "#2d3748" }}>{rate.toFixed(1)} mL/hr × {durationHrs} hr</span>}</div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", padding: "12px", borderTop: "1px solid #e2e8f0" }}>
            <NutrientChip label="Energy" value={totalKcal} unit="kcal/day" color="#e67e22" />
            <NutrientChip label="Protein" value={aaG} unit={`g/day (${patientWtKg > 0 ? (aaG/patientWtKg).toFixed(1) : "—"} g/kg)`} color="#8e44ad" />
            <NutrientChip label="Volume" value={totalVol} unit="mL/day" color="#3498db" />
          ...
          <div style={{ padding: "0 12px 12px", fontSize: "0.7rem", color: "#718096" }}>
            <div style={{ display: "flex", gap: "15px", flexWrap: "wrap" }}>
              ...
              <span>• Final Dex: <strong>{finalDexPct.toFixed(1)}%</strong> {finalDexPct > 25 ? <span style={{ color: "#e53e3e" }}> (Central access required)</span> : "(Safe for peripheral if PPN)"}</span>
