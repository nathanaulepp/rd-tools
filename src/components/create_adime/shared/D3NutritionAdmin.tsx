...
// Simplified macro types: only Premade or Compounded
...
function CollapseHeader({ label, expanded, onToggle, accent = "#3498db", badge = null }: CollapseHeaderProps) {
  return (
    <div onClick={onToggle} style={{
      ...
      marginBottom: expanded ? 0 : "0.5rem", transition: "border-radius 0.2s",
    }}>
...
interface AmountRateInputProps {
  ...
  onAmount: (val: string) => void; onAmountUnit: (val: string) => void; onRateUnit: (val: string) => void;
}
...
// ─── Helper: derive how many infusion-rate fields to show ────────────────────
// TNA → 1 combined rate
// 2-in-1 + Separate Lipid → 2 rates: (Dext+AA combined), (Lipid)
// 3 Fully Separated / Module-Based / Transitioning → 3 individual rates
...
function ENFeedCard({ feed, idx, onChange, onRemove, savedFormulas, onAddFormula }: ENFeedCardProps) {
  ...
  const handleFormulaChange = (val: string) => {
    if (val === "+ Add new formula...") {
      ...
      if (name && name.trim()) { onAddFormula(name.trim()); update("formula", name.trim()); }
...
function D32Enteral() {
  ...
  const addFeed = () => { setEnFeeds([...enFeeds, makeENFeed(nextId)]); setNextId(nextId + 1); };
  ...
  return (
    <div className="card" style={{ marginBottom: "1.5rem" }}>
      ...
      {enFeeds.map((feed, idx) => (
        <ENFeedCard key={feed.id} feed={feed} idx={idx} onChange={updated => updateFeed(feed.id, updated)} onRemove={() => removeFeed(feed.id)} savedFormulas={savedFormulas} onAddFormula={addFormula} />
      ))}
      {enFeeds.length > 1 && (
            ...
            {fields.map(f => (
              <FieldRow key={f.key} label={f.label}>
                <div style={{ display: "flex", gap: "4px" }}>
                  ...
                  <select value={values[f.key]?.unit || "mEq"} onChange={e => onChange(f.key, "unit", e.target.value)} style={{ padding: "5px 4px", border: "1px solid #e2e8f0", borderRadius: "4px", fontSize: "0.78rem", width: "58px" }}>
                    {AMOUNT_UNITS.map(u => <option key={u}>{u}</option>)}
                  </select>
                  <select value={values[f.key]?.rate || "per day"} onChange={e => onChange(f.key, "rate", e.target.value)} style={{ padding: "5px 4px", border: "1px solid #e2e8f0", borderRadius: "4px", fontSize: "0.78rem", width: "72px" }}>
                    {RATE_UNITS.map(u => <option key={u}>{u}</option>)}
                  </select>
                </div>
              </FieldRow>
    ...
    <div style={{ border: "1px solid #e2e8f0", borderRadius: "8px", marginBottom: "1rem", overflow: "hidden" }}>
      ...
      {isExpanded && (
        <div style={{ padding: "1rem", borderTop: "1px solid #e2e8f0", background: "#fff" }}>
          ...
          {/* ── Route / Access / Delivery / Goal — 4-col grid ── */}
          {/* Delivery is now wedged between Access and Goal */}
          ...
          <div style={{ background: "#faf5ff", border: "1px solid #e9d8fd", borderRadius: "8px", padding: "1rem", marginBottom: "1rem" }}>
            ...
            {/* TNA: single combined infusion rate shown above all macros */}
            {rateMode === "tna" && (
              <div style={{ marginBottom: "1rem", display: "flex", alignItems: "center", gap: "12px", background: "#ede9fe", borderRadius: "6px", padding: "0.6rem 1rem" }}>
                <span style={{ fontSize: "0.82rem", fontWeight: 700, color: "#6b46c1" }}>TNA Infusion Rate:</span>
            ...
            {/* 2-in-1: combined Dext+AA rate + separate Lipid rate (shown in lipid row) */}
            {rateMode === "twoplusone" && (
              <div style={{ marginBottom: "1rem", display: "flex", alignItems: "center", gap: "12px", background: "#ede9fe", borderRadius: "6px", padding: "0.6rem 1rem" }}>
                <span style={{ fontSize: "0.82rem", fontWeight: 700, color: "#6b46c1" }}>Dextrose + AA Combined Rate:</span>
                <NumInput value={feed.combinedRate} onChange={v => update("combinedRate", v)} placeholder="mL/hr" style={{ width: "90px" }} />
                <span style={{ fontSize: "0.8rem", color: "#718096" }}>mL/hr</span>
              </div>
            )}
...
function D33Parenteral() {
  ...
  const addBag = () => { setPnBags([...pnBags, makePNFeed(nextId)]); setNextId(nextId + 1); };
  ...
  return (
    <div className="fade-in">
      <div style={{ display: "flex", gap: "4px", marginBottom: "1.5rem", background: "#f4f7f6", padding: "4px", borderRadius: "10px", width: "fit-content" }}>
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: "8px 22px", border: "none", borderRadius: "7px", cursor: "pointer", fontWeight: 600, fontSize: "0.88rem", transition: "all 0.2s", background: activeTab === tab.id ? tab.color : "transparent", color: activeTab === tab.id ? "#fff" : "#718096", boxShadow: activeTab === tab.id ? "0 2px 6px rgba(0,0,0,0.15)" : "none" }}>
            {tab.label}
          </button>
        ))}
      </div>
