...
export const DIETARY_CATEGORIES = [
  ...
  { id: "D8", title: "D8: Physical Activity & Function" },
...
export default function DietaryDomain({ dietary, setDietary, activeSubDomain }: any) {
  ...
  const updateRecall = (field: string, val: string) => setDietary({ ...dietary, recall: { ...dietary.recall, [field]: val } });
  ...
  const renderContent = () => {
    switch (activeSubDomain) {
            ...
            <div className="card">
              ...
              <div className="stepper-container">
                ...
                <div className="input-group">
                  <textarea 
                    value={dietary.recall[["breakfast", "lunch", "dinner", "snacks"][recallStep] as keyof typeof dietary.recall]} 
                    onChange={e => updateRecall(["breakfast", "lunch", "dinner", "snacks"][recallStep], e.target.value)}
      ...
      case "D3":
        return (
          <div className="card">
            <h4 className="mb-1">D31: Oral Diet Order vs Intake</h4>
            <div className="split-pane mb-1">
              <div className="pane-side">
                <div className="input-group">
                  <label>Prescribed Diet Order</label>
                  <textarea value={dietary.dietOrder} onChange={e=>handleUpdate("dietOrder", e.target.value)} readOnly={false} style={{ background: '#edf2f7' }} />
                </div>
              </div>
              <div className="pane-side">
                <div className="input-group">
                  <label>Actual Intake Documented</label>
                  <textarea value={dietary.actualIntake} onChange={e=>handleUpdate("actualIntake", e.target.value)} placeholder="Compare with prescribed..." />
                </div>
              </div>
            </div>
            <div className="input-group mt-1">
              <label>D32/D33: Enteral (EN) / Parenteral (PN) Nutrition</label>
              <textarea value={dietary.enteralPN} onChange={e=>handleUpdate("enteralPN", e.target.value)} />
            </div>
          </div>
        );
      case "D5":
            ...
            <div className="input-group" style={{ maxWidth: '400px', marginTop: '1.5rem' }}>
              ...
              <div className="slider-container">
                <input 
                  ...
                  value={dietary.readiness} 
                  onChange={e => handleUpdate("readiness", e.target.value)} 
                />
                <div className="slider-labels">
                  <span>1 (Not Ready)</span>
                  <span style={{ fontWeight: 'bold', color: '#2c3e50', fontSize: '1.2rem' }}>{dietary.readiness}</span>
